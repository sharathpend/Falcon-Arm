...
void ntt_rewrite_forward(uint16_t a[FALCON_N])
{
...
void ntt_rewrite_inverse(uint16_t a[FALCON_N])
{
...
#error "See config.h, FALCON_N is not supported"
    ...
    {
        // a[j] = mq_montymul(f, a[j]);
        a[j] = ((uint32_t)f * a[j]) % Q;
    }
}
...
int test_ntt(void)
{
    uint16_t gold[FALCON_N], test[FALCON_N];
    uint16_t temp;
