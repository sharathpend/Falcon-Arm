...
# set -e 
while inotifywait -e close_write *.c; do 
    make -j4
    build/kat512fpu > /tmp/neon;
    diff /tmp/ref /tmp/neon
    md5sum *.rsp
done 

# while inotifywait -e close_write sign.c; do 
#     make; build/kat512fpu > /tmp/ref
# done 
