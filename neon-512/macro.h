...
vfpr_expm_p63(float64x2_t x, float64x2_t ccs)
{
    ...
    uint64x2_t ret;
    float64x2x4_t neon_c07, neon_exp[3]; // C[0->7]
    ...
    // vloadx4(neon_exp[0], &C_expm[0]);
    // vloadx4(neon_exp[1], &C_expm[8]);
    // vloadx4(neon_exp[2], &C_expm[16]);

    // vfms(y, neon_exp[0].val[1], neon_exp[0].val[0], x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[0].val[2], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[0].val[3], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[1].val[0], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[1].val[1], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[1].val[2], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[1].val[3], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[2].val[0], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[2].val[1], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[2].val[2], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // vfms(y, neon_exp[2].val[3], y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    // z = vdupq_n_f64(1.0);
    // vfms(y, z, y, x);
    // printf("y = %.20f|%.20f\n", y[0], y[1]);
    float64x2_t x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12,
    y2, y3;
    ...
    vfma(y1, vdupq_n_f64(C_expm[7]), vdupq_n_f64(C_expm[10]), x3);
    vfma(y1, vdupq_n_f64(C_expm[4]), y1, x3);
    vfma(y1, vdupq_n_f64(C_expm[1]), y1, x3);

    vfma(y2, vdupq_n_f64(C_expm[8]), vdupq_n_f64(C_expm[11]), x3);
    vfma(y2, vdupq_n_f64(C_expm[5]), y2, x3);
    vfma(y2, vdupq_n_f64(C_expm[2]), y2, x3);

    vfma(y3, vdupq_n_f64(C_expm[9]), vdupq_n_f64(C_expm[12]), x3);
    vfma(y3, vdupq_n_f64(C_expm[6]), y3, x3);
    vfma(y3, vdupq_n_f64(C_expm[3]), y3, x3);
