...
static void fullcycle(uint8_t *public_key, uint8_t *secret_key,
  ...
  if (PQCLEAN_FALCON1024_AARCH64_crypto_sign_keypair(public_key, secret_key)) {
    printf("keygen error. Exiting. %u\n", count);
    exit(-1);
  }
  if (PQCLEAN_FALCON1024_AARCH64_crypto_sign_signature(
          signature, &signature_len, message, message_len, secret_key)) {
    print(signature, signature_len, "signature");
    print(secret_key, PQCLEAN_FALCON1024_AARCH64_CRYPTO_SECRETKEYBYTES, "secret_key");
    printf("sign error. Exiting. %u\n", count);
    exit(-1);
  }
  if (PQCLEAN_FALCON1024_AARCH64_crypto_sign_verify(
...
int main(void) {
  ...
  public_key = malloc(PQCLEAN_FALCON1024_AARCH64_CRYPTO_PUBLICKEYBYTES);
  secret_key = malloc(PQCLEAN_FALCON1024_AARCH64_CRYPTO_SECRETKEYBYTES);
  signature = malloc(PQCLEAN_FALCON1024_AARCH64_CRYPTO_BYTES);
