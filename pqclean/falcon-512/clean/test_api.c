...
static void fullcycle(uint8_t *public_key, uint8_t *secret_key,
  ...
  if (PQCLEAN_FALCON512_CLEAN_crypto_sign_keypair(public_key, secret_key)) {
  ...
  if (PQCLEAN_FALCON512_CLEAN_crypto_sign_signature(
    ...
    print(secret_key, PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES, "secret_key");
...
//   if (count == 170) {
//     print(public_key, PQCLEAN_FALCON512_CLEAN_CRYPTO_PUBLICKEYBYTES, "public_key");
//     print(secret_key, PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES, "secret_key");
//     // print(signature, signature_len, "signature");
//     exit(0);
//   }
...
int main(void) {
  ...
  public_key = malloc(PQCLEAN_FALCON512_CLEAN_CRYPTO_PUBLICKEYBYTES);
  secret_key = malloc(PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES);
  signature = malloc(PQCLEAN_FALCON512_CLEAN_CRYPTO_BYTES);
